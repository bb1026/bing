...
<div id="inputBox">
  ...
  <input type="number" id="hourInput" placeholder="输入加班小时" min="0" step="0.1">
...
function computeStats() {
  ...
  工作日加班: \${wday}小时 | $\${showSalary ? (wday * salary.hourlyRate * raw.settings.rateWeekday).toFixed(2) : '--'}<br>
    星期六加班: \${wsat}小时 | $\${showSalary ? (wsat * salary.hourlyRate * raw.settings.rateSaturday).toFixed(2) : '--'}<br>
    星期日加班: \${wsun}小时 | $\${showSalary ? (wsun * salary.hourlyRate * raw.settings.rateSunday).toFixed(2) : '--'}<br>
    \${showSalary ? \`基础薪资: $\${salary.base.toFixed(2)}<br>
    住房津贴: $\${salary.allowance.toFixed(2)}<br>
    全勤奖金: $\${salary.attendance.toFixed(2)}<br>
    自定义项目: $\${salary.custom.toFixed(2)}\` : '工资信息已隐藏'}
 \`;
}
  ...
  while (temp <= endDate) {
    ...
    cal.innerHTML += \`
      <div class="day \${w === 0 ? 'sun' : w === 6 ? 'sat' : ''}" 
           ...
           style="\${dayStyle}">
        \${temp.getDate()}<br>
        <span style="color:#666;font-size:14px">\${hrs}h</span>
      </div>\`;
  ...
  for (let d = new Date(rng.start); d <= new Date(rng.end); d.setDate(d.getDate() + 1)) {
    ...
    content.innerHTML += \`
      <div class="batch-edit-row">
        ...
        <input type="number" class="batch-edit-input" 
               data-date="\${dt}" 
               value="\${hrs}" 
