...
async function createTable(
  ...
  searchStopButton.widthWeight = 34;
  ...
  if (useLocation) {
    ...
    if (nearestStops.length) {
      for (const stop of nearestStops) {
        ...
        stopRow.addText(`${stop.Description} (${stop.BusStopCode})`);
        // 点击后查看该站点的所有巴士信息
    ...
    for (const { busstop, stopCode, busCodes } of myBusCodes) {
      ...
      await addBusArrivalRows(table, stopCode, busCodes);
    }
  }
...
async function addBusArrivalRows(table, stopCode, allowedBusCodes = null) {
  ...
  for (const service of stopArrivalInfo.Services) {
    ...
    // **点击时显示首末班车时间**
    row.onSelect = async () => {
      await showBusFirstLastTimes(stopCode, service.ServiceNo);
    };
...
async function showBusFirstLastTimes(stopCode, busCode) {
  ...
  // **第一行：站点信息**
  const stopRow = new UITableRow();
  stopRow.addText(`站点: ${stopCode}`).widthWeight = 50;
  table.addRow(stopRow);
